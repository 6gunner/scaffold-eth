...
describe("Staker dApp", () => {
  ...
  describe("Test buyTokens() method", () => {
    ...
    it("buyTokens success!", async () => {
      ...
      const vendorTokenBalance = await tokenContract.balanceOf(
        vendorContract.address
      );
      ...
      const vendorBalance = await ethers.provider.getBalance(
        vendorContract.address
      );
  ...
  describe("Test withdraw() method", () => {
    ...
    it("withdraw reverted because called by not the owner", async () => {
      await expect(vendorContract.connect(owner).withdraw()).to.be.revertedWith(
  ...
  describe("Test sellTokens() method", () => {
    it("sellTokens reverted because tokenAmountToSell is 0", async () => {
      ...
      await expect(
        vendorContract.connect(addr1).sellTokens(amountToSell)
      ).to.be.revertedWith("Specify an amount of token greater than zero");
    });
    ...
    it("sellTokens success", async () => {
      ...
      await tokenContract
        .connect(addr1)
        .approve(vendorContract.address, amountToSell);

      // check that the Vendor can transfer the amount of tokens we want to sell
      const vendorAllowance = await tokenContract.allowance(
        addr1.address,
        vendorContract.address
      );
      expect(vendorAllowance).to.equal(amountToSell);

      const sellTx = await vendorContract
        .connect(addr1)
